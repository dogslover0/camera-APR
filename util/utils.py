...
####NOTE
#### os.path.realpath 获取绝对路径; os.path.split 将文件名和路径分开，使用文件的绝对路径作为输入
#### logging 分为5个等级，CRITICAL > ERROR > WARNING > INFO > DEBUG 默认为WARNING，低于WARNING的不会打印在屏幕上
####
...
def init_logger():
    ...
    with open(join(path, 'log_config.json')) as json_file:
        ...
        # Creating logs' folder is needed 日志文件放在out文件夹中
